...
class ScraperCoupang(Scraper):
    ...
    def findCandidates(self, html: str):
        ...
        for item in items:
            ...
            product_preview = ProductPreview(product_title, product_url, original_price, normal_discount_price, card_discount_percent,
                                             normal_card_total_discount_percent, is_more_discount_exist)
    ...
    def getCardDiscountAmount(self, driver: WebDriver, product: ProductPreview):
        if product.card_discount_percent != 0:
            ...
            driver.switch_to.frame(driver.find_element_by_xpath("//iframe[@class='card-benefit-popup__content-iframe']"))
    ...
    def getMoreDiscountAmount(self, driver: WebDriver, product: ProductPreview):
        if product.is_more_discount_exist:
            ...
            for coupon in driver.find_elements_by_xpath("//span[@class='prod-coupon-price']"):
                max_more_discount_amount = max(int(coupon.get_attribute("innerHTML").replace(",", "").replace(" ", "").replace("원", "")),max_more_discount_amount)
